...
public class OpponentAI : MonoBehaviour
{
    ...
    public FightingController[] fighttingController;
    ...
    void Update(){
        ...
        for(int i = 0; i < fighttingController.Length; i++){
            if(players[i].gameObject.activeSelf && Vector3.Distance(transform.position, players[i].position) <= attackRadius)
            {
                ...
                if(Time.time - lastAttackTime > attackCooldown)
                {
                    ...
                    //Play hit/damage animations on the player
                    fighttingController[i].StartCoroutine(fighttingController[i].PlayHitDamageAnimation(attackDamages));
